...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $existSql = "SELECT * FROM `shoppingcart` WHERE pizzaId = '$itemId' AND `userId`='$userId'";
        ...
        if($numExistRows > 0){
            ...
            $sql = "INSERT INTO `shoppingcart` (`pizzaId`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())";   
    ...
    if(isset($_POST['removeItem'])) {
        $itemId = $_POST["itemId"];
        $sql = "DELETE FROM `shoppingcart` WHERE `pizzaId`='$itemId' AND `userId`='$userId'";   
        $result = mysqli_query($conn, $sql);
    ...
    if(isset($_POST['checkout'])) {
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                $addSql = "SELECT * FROM `shoppingcart` WHERE userId='$userId'"; 
                ...
                $deletesql = "DELETE FROM `shoppingcart` WHERE `userId`='$userId'";   
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        ...
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `shoppingcart` SET `itemQuantity`='$qty' WHERE `pizzaId`='$pizzaId' AND `userId`='$userId'";
