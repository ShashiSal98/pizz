...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['createUser'])) {
        ...
        if($numExistRows > 0){
            ...
            if(($password == $cpassword)){
                ...
                $sql = "INSERT INTO `admin` ( `username`, `firstName`, `lastName`, `email`, `phone`, `userType`, `password`, `joinDate`) VALUES ('$username', '$firstName', '$lastName', '$email', '$phone', '$userType', '$hash', current_timestamp())";   
                $result = mysqli_query($conn, $sql);
                if ($result){
    ...
    if(isset($_POST['editUser'])) {
        ...
        $sql = "UPDATE `admin` SET `firstName`='$firstName', `lastName`='$lastName', `email`='$email', `phone`='$phone', `userType`='$userType' WHERE `id`='$id'";   
        $result = mysqli_query($conn, $sql);
        if ($result){
